...
fn main() {
    ...
    X.slice_mut((.., 1..)).set(data.slice((.., 1..)));
    let y = data.col(0);

    let (mu, sigma) = timeit!("Normalization", {
        normalize(X.slice_mut((.., 1..)))
    });

    println!("mean: {:?}", mu);
    println!("std deviation: {:?}\n", sigma);

    let mut theta = ColVec::zeros(n + 1);

    let alpha = 0.01;
    let max_niters = 100_000;

    let niters = timeit!("Gradient descent", {
        descent(X.slice(..), y, theta.slice_mut(..), alpha, max_niters)
    });
...
fn normalize(mut X: SubMatMut<f64>) -> (Vec<f64>, Vec<f64>) {
